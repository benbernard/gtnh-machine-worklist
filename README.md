...
Under development. Windows, Linux, and macOS builds pass 29 automated tests and produce identical mod JARs. Live GTNH 2.8.4 checks verify selected-group import, inventory deduction, circuit-dependent readiness, recipe navigation, the three tabs, the missing-input pane, and exact/half/cleared manual completion. Player-grid and handheld Adventure Backpack one-batch transfers passed, as did a client-only EMBU connection and read-only worklist check. Three-column recipes, occupied-grid/cursor/full-inventory guards and multiplayer transfers still need live checks. See the validation record for the exact tested revisions.
...
Keyboard navigation: **1/2/3** selects Machines/Crafting/All. **Up/Down** selects a queue row; **Enter** opens it. In recipe details, **F** requests one NEI crafting batch using the same checks as the button, **N** opens the NEI recipe, and arrows scroll. **R** toggles ready-only; **M** toggles the missing view in smaller windows. **Escape** returns. Holding F does not repeat the request.
