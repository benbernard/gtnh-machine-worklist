        ...
        if (button.id == 1) mc.displayGuiScreen(
            new InformationScreen(
                ...
                Arrays.asList(
                    ...
                    "For your own plan, search in NEI, press R on the target and choose the recipe you want. Use NEI's Save Recipe Tree control (star) to save the chosen tree to bookmarks. NEI also provides Add Bookmark with Recipe; use its displayed key binding if yours was changed.",
                    "In the bookmark panel, right-click the group bracket to toggle Crafting Chain mode. For the default ungrouped page, right-click its page header. Ctrl + mouse wheel changes the target quantity. Open the worklist again to import changes.",
                    ...
                    "Craft 1 batch (F) runs once. Craft all ready (G) runs the selected recipe up to the displayed quantity and stops if conditions change. Escape closes the container and cancels further batches. Machines are performed manually.",
