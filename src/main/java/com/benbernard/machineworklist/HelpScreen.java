...
final class HelpScreen extends WorklistGui {
    ...
    private static final String[][] TEXT = { {
        "FIRST TRY: Open inventory and press F10 (or your rebound Worklist key). Press B to choose a group if a worklist opens directly. In the group picker, E opens an eight-table example using this pack's real recipe. It gives no items and changes no NEI bookmarks. Tables already owned reduce the target.",
        ...
        "F10 imports the hovered crafting group, opens the only group directly, or shows the group picker. The picker lists groups on the current NEI page. After changing recipes, quantities or pages in NEI, reopen the worklist to import them. You choose recipes; this mod does not choose replacements." },
        ...
        { "A disabled F or G button means the request cannot start now. Press its shortcut for a full explanation, or Tab to that button and press Enter. In recipe details B also shows status. A downstream recipe can be WAITING while G is available to make its ingredients.",
            ...
            "GRID OR CURSOR OCCUPIED: Put the cursor item in a slot and empty the crafting grid and result slot before F10. The entry warning leaves your items in their original screen. Stored supplies in a supported backpack can stay.",
        ...
        { "Open the worklist from survival inventory for 2x2 recipes, from a crafting table for 3x3, or from the supported Adventure Backpack 1.3.13-GTNH GUI. Supplies come from player inventory/hotbar plus ordinary storage in that open backpack. Its bottom-right 3x3 crafting slots must be empty. Hidden mirror, tool and tank slots are not supplies.",
            "F10 is rebindable under Options > Controls > GTNH Machine Worklist. Open inventory once to initialize NEI before using /machineworklist or /machineworklist <group number>. The command opens an inventory worklist, so use F10 from a table/backpack for its grid and storage.",
    ...
    public void drawScreen(int x, int y, float ticks) {
        ...
        drawString(fontRendererObj, "1-6: topic. Tab + Enter: buttons. Esc: back.", 12, height - 18, 0xa9b7cb);
