...
final class HelpScreen extends WorklistGui {
    ...
    private static final String[][] TEXT = { {
        ...
        { "A disabled F or G button means the request cannot start now. Press its shortcut for a full explanation, or Tab to that button and press Enter. In recipe details B also shows status. A downstream recipe can be WAITING while G is available to make its ingredients.",
            ...
            "RETURNING TOOLS: Files, screwdrivers and other reusable ingredients can briefly remain visible after crafting, especially in the backpack. A clear grid/cursor resumes on the next client tick; it does not wait for the whole inventory to stop changing. Actual leftovers still get a bounded wait and recovery into player inventory. Leave a free player slot. Cleanup never takes another result. If it stops, inspect the cursor and bottom-right backpack grid, then close/reopen the actual container if a tool looks stuck. A timed-out transfer is unconfirmed; reopening recalculates actual stock.",
