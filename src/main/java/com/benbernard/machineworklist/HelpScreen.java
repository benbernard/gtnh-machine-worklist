...
final class HelpScreen extends WorklistGui {
    ...
    private static final String[][] TEXT = { {
        ...
        { "The queue shows remaining work after inventory and recorded stock are credited. A run is one execution of a recipe; outputs show how many items or mB those runs produce. The target is a total to have available, not an extra amount to craft.",
            "Machines (1), Crafting (2) and All (3) filter the rows. Machine operations show READY first, then WAITING. Each group is sorted by remaining runs, largest first, including partially ready operations. This also orders machine rows in All. R toggles ready-only rows. These filters do not limit Craft group or change the overall missing-input totals.",
            ...
            "VIRTUAL CIRCUITS: A reusable Programmed / Programmable Circuit is the machine's circuit setting. It is always available and never listed as missing. Details keep its configuration number: set that number in the machine's ghost circuit slot. Real tools, molds and consumed circuit ingredients still need physical items.",
            ...
            "Detail rows include outputs, inputs, reusable tools and upstream recipes. Click an upstream row, or select it with arrows and Enter, to inspect it. Click other item rows, missing inputs or the stock screen's item name to view that item's NEI recipes. Queue rows still open their worklist details. Scroll to read every explanation and recipe note. Escape returns to the queue." },
        { "F / Craft 1 batch runs the selected recipe exactly once, using physical ingredients already available. One batch may produce several items. It does not make missing ingredients first.",
            "G / Craft chain in recipe details includes that recipe and its chosen upstream dependencies. It prefers large batches of the chosen intermediates before switching to downstream recipes, up to 64 recipe runs per transfer. For example, it can make 128 rods before combining 64 long rods. Remaining demand, ingredients, tool durability and space bound each batch. If a producer is blocked, a ready downstream recipe can run to free space. Unrelated targets are excluded.",
            ...
            "RETURNED TOOLS AND BATCHES: A file or screwdriver that returns after crafting can serve multiple batches. Tool-using previews check up to 64 batches per transfer and limit that count by remaining durability. All matching tools must survive the chosen count; a nearly broken spare may shorten a transfer. Separate tool slots still need separate tools. After each transfer, the next preview uses current stock and tool damage.",
            ...
            "Escape while transfers are running closes the container and stops further crafting. Finished transfers remain in inventory. Reopening and pressing G calculates the remaining work from current stock. Open backpack or station storage provides overflow space automatically. If all usable space fills, move items elsewhere and resume; record stock moved outside the open container in Available stock." },
        ...
        { "A disabled F or G button means the request cannot start now. Press its shortcut for a full explanation, or Tab to that button and press Enter. In recipe details B also shows status. A downstream recipe can be WAITING while G is available to make its ingredients.",
            "MISSING INPUTS: Read the named items, tool configurations and amounts. Supply them, craft chosen upstream recipes, or perform the machine operation shown in NEI. Only the open backpack or Tinkers' station's exposed attached storage supplies extra ingredients. Other chests, machines and closed backpacks are not scanned.",
            ...
            "GRID TOO SMALL: Close the worklist, open a real 3x3 table or supported backpack, then press F10 again. INVENTORY SPACE: Free player slots or usable open storage. F/G may move whole player stacks into open storage to make room for transfers, outputs and returned tools. The held hotbar item stays in place. Previews never move items.",
            ...
            "RETURNING TOOLS: Files, screwdrivers and other reusable ingredients can briefly remain visible after crafting, especially in the backpack. Each bulk transfer waits for server confirmation before continuing or recovering tools; it still crafts up to 64 runs at once. A clean confirmed grid resumes promptly, without waiting for unrelated inventory changes. The backpack mirror refreshes from its real slots after confirmation. Recognized leftovers return to player inventory without an extra quiet delay; recovery attempts are bounded. Leave player or usable open-storage space. Cleanup never takes another result. If it stops, inspect the cursor and bottom-right backpack grid, then close/reopen the actual container if a tool looks stuck. A timed-out transfer is unconfirmed; reopening recalculates actual stock.",
            "PAUSED OR TRANSFER STOPPED: Read the result, inspect actual inventory/grid contents, correct the blocker and start again. Completed items are kept. No missing recipes are invented. Closing the container, death or disconnection stops further transfers, including tool-return recovery." },
        { "Open the worklist from survival inventory for 2x2 recipes, from a crafting table for 3x3, or from the supported Adventure Backpack 1.3.13-GTNH GUI. Supplies come from player inventory/hotbar plus ordinary storage in that open backpack. Its bottom-right 3x3 crafting slots must be empty. Hidden mirror, tool and tank slots are not supplies.",
            "The queue header names the active crafting grid. Open a Tinkers' Crafting Station before F10 to use its 3x3 grid. Exposed attached storage supplies ingredients and output overflow, respecting slot permissions. Backpack ordinary storage does both too. Overflow never uses the crafting grid, hidden mirrors, utility or tank slots. Storage must accept items and allow them to be taken back. A changed/refused transfer stops crafting; check the cursor and storage before resuming. Reopening remembers the chain and position, while using the container you just opened. A closed backpack is never selected instead of that table.",
